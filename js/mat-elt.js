...
//               MAT.terrain (optional, for USGS elevation API)
// 
// Updated: Based on RT-600/SAR-DF 517 specifications (±5° RMS native accuracy)
// and blind testing across multiple terrain scenarios.
// 
// Terrain Integration:
//   - When MAT.terrain is available and online, uses USGS EPQS to classify
//     terrain automatically (flat/moderate/mountainous)
//   - Falls back to keyword detection in observation notes when offline
//   - Use computeProbableAreaAsync() for terrain-enhanced results
//   - Use computeProbableArea() for offline-only operation
// ==========================================================================
...
(function() {
  ...
  const DEFAULT_SETTINGS = {
    ...
    terrainMode: 'auto'  // 'flat', 'moderate', 'mountainous', or 'auto'
  };
  
  // ========================================
  // DF BEARING CONSTANTS (RT-600 Based)
  // ========================================
  // RT-600/SAR-DF 517 has ±5° RMS native accuracy
  // Signal strength affects response time, NOT bearing accuracy
  // Terrain/multipath is the primary source of additional error
  
  const DF_BEARING_CONSTANTS = {
    // RT-600 = ±5° + small terrain margin = 10° baseline
    // This is for GOOD conditions (flat terrain, clear line of sight)
    MIN_BEARING_SIGMA: 10,
    
    // Trust the instrument (no blanket penalty)
    BEARING_SIGMA_SCALE: 1.0,
    
    // Maximum bearing likelihood contribution (prevents single obs domination)
    MAX_BEARING_LOG_LIKELIHOOD: -0.005,
    
    // Terrain-related keywords that trigger additional uncertainty
    // These represent REAL degradation sources per the RT-600 manual
    TERRAIN_KEYWORDS: ['canyon', 'valley', 'ridge', 'mountain', 'multipath', 
                       'terrain', 'masking', 'intermittent', 'null', 'reflection'],
    
    // Additional sigma when terrain keywords detected
    // Multipath can easily add 20-30° of error in canyons
    TERRAIN_SIGMA_BOOST: 20,
    
    // Weak signal handling - per RT-600 manual, weak signals give correct
    // bearings but with slower response time. However, in terrain, weak
    // signals are MORE susceptible to multipath corruption.
    WEAK_SIGNAL_TERRAIN_BOOST: 10
  };
  
  // ========================================
  // TERRAIN MODE ADJUSTMENTS
  // ========================================
  
  /**
   * Get terrain-adjusted DF bearing constants
   * Allows mission commanders to indicate operating area terrain type
   * 
   * @param {string} terrainMode - 'flat', 'moderate', 'mountainous', or 'auto'
   * @returns {Object} Adjusted DF bearing constants
   */
  function getTerrainAdjustedConstants(terrainMode = 'auto') {
    const base = { ...DF_BEARING_CONSTANTS };
    
    switch (terrainMode) {
      case 'flat':
        // Trust the RT-600's ±5° accuracy
        // Plains, desert, open water, low hills
        base.MIN_BEARING_SIGMA = 8;      // 5° instrument + 3° margin
        base.TERRAIN_SIGMA_BOOST = 5;    // Minimal terrain effect
        base.WEAK_SIGNAL_TERRAIN_BOOST = 5;
        break;
        
      case 'moderate':
        // Foothills, forests, rolling terrain
        // Some multipath possible but not severe
        base.MIN_BEARING_SIGMA = 12;
        base.TERRAIN_SIGMA_BOOST = 15;
        base.WEAK_SIGNAL_TERRAIN_BOOST = 10;
        break;
        
      case 'mountainous':
        // Canyons, alpine, deep valleys
        // Severe multipath expected
        base.MIN_BEARING_SIGMA = 15;
        base.TERRAIN_SIGMA_BOOST = 25;
        base.WEAK_SIGNAL_TERRAIN_BOOST = 15;
        break;
        
      case 'auto':
      default:
        // Use default constants, let keyword detection handle it
        // Good default for mixed or unknown terrain
        break;
    }
   ...
   * 
   * UPDATED: Based on RT-600/SAR-DF 517 specifications (±5° RMS native accuracy)
   * - Signal strength affects RANGE estimation, not bearing accuracy
   * - Terrain/multipath is the primary source of bearing error
   * - Weak signals only penalized when terrain issues suspected
   * 
   * @param {number} lat - Grid cell latitude
   * @param {number} lon - Grid cell longitude
   * @param {Object} obs - Observation data
   * @param {Object} settings - ELT settings (r10, k, terrainMode)
   * @param {number} uncertaintyPct - Uncertainty percentage
  ...
  function observationLogLikelihood(lat, lon, obs, settings, uncertaintyPct, uncertaintyMin) {
    ...
    // Get terrain-adjusted constants
    const dfConst = getTerrainAdjustedConstants(settings.terrainMode || 'auto');
    
    // ========================================
    // RANGE LIKELIHOOD (unchanged - working well)
    // ========================================
    let lRange;
    ...
    // ========================================
    // BEARING LIKELIHOOD (RT-600 based update)
    // ========================================
    if (obs.bearing !== null) {
      ...
      // Start with RT-600 native accuracy (±5°) plus baseline margin
      // If operator provided bearingAcc, use it but enforce minimum
      let bearingSigma = Math.max(
        obs.bearingAcc || 10,  // Default to 10° if not specified
        dfConst.MIN_BEARING_SIGMA
      );
      
      // Apply general scale factor (1.0 = trust the instrument)
      bearingSigma *= dfConst.BEARING_SIGMA_SCALE;
      
      // Check for terrain indicators in notes
      let terrainSuspected = false;
      if (obs.notes) {
        const notesLower = obs.notes.toLowerCase();
        terrainSuspected = dfConst.TERRAIN_KEYWORDS.some(kw => 
          notesLower.includes(kw)
        );
        
        if (terrainSuspected) {
          // Significant boost for terrain - multipath can add 20-30°
          bearingSigma += dfConst.TERRAIN_SIGMA_BOOST;
        }
      }
      
      // Weak signal handling:
      // Per RT-600 manual, weak signals still give accurate bearings
      // BUT in terrain, weak signals are more likely to be multipath-corrupted
      // Only add uncertainty if signal is weak AND terrain is suspected
      if (obs.strength && obs.strength <= 3) {
        if (terrainSuspected || bearingSigma > 15) {
          // Weak signal + terrain = extra uncertainty
          bearingSigma += dfConst.WEAK_SIGNAL_TERRAIN_BOOST;
        }
        // Note: In flat terrain with weak signal, we trust the RT-600's
        // ±5° accuracy - it just takes longer to stabilize
      }
      
      // Calculate bearing likelihood
      let lBearing = -0.5 * Math.pow(dTheta / bearingSigma, 2);
      
      // Cap the bearing likelihood contribution
      // Prevents any single "perfect" bearing from overwhelming the solution
      lBearing = Math.min(lBearing, dfConst.MAX_BEARING_LOG_LIKELIHOOD);
      
      logL += lBearing;
    }
    
    // ========================================
    // RADIO HORIZON SOFT CAP (unchanged)
    // ========================================
   ...
   * @param {Object} [params.settings] - ELT settings (r10, k, resolution, confidenceMode, terrainMode)
  ...
  function computeProbableArea(params) {
    ...
    const obsData = activeObs.map(o => ({
      ...
      bearingAcc: parseFloat(o.bearingAccuracy) || 10,  // Default changed from 20 to 10 (RT-600 based)
      agl: parseFloat(o.altAGL) || 1500,
      source: o.source || 'audio',
      notes: o.notes || ''  // Include notes for terrain keyword detection
    }));
    ...
    return {
      ...
      terrainMode: settings.terrainMode || 'auto',
      grid: grid,
      bounds: bounds,
  ...
  // ========================================
  // TERRAIN-ENHANCED COMPUTATION (Async)
  // ========================================
  
  /**
   * Compute ELT probable area with automatic terrain analysis
   * 
   * This is the enhanced version that uses USGS EPQS for terrain
   * classification when online, falling back to keyword detection
   * when offline.
   * 
   * @param {Object} params - Same as computeProbableArea, plus:
   * @param {boolean} [params.useTerrainApi=true] - Whether to try USGS API
   * @returns {Promise<Object>} Result object with terrain analysis included
   */
  async function computeProbableAreaAsync(params) {
    const {
      observations,
      settings = DEFAULT_SETTINGS,
      useTerrainApi = true
    ...
    // If terrain module not available or disabled, use sync version
    if (!useTerrainApi || !window.MAT?.terrain?.analyzeTerrain) {
      return computeProbableArea(params);
    }
    ...
    // Calculate center of observations for terrain query
    const lats = activeObs.map(o => o.lat !== undefined ? o.lat : 0);
    const lons = activeObs.map(o => o.lon !== undefined ? o.lon : 0);
    const centerLat = lats.reduce((a, b) => a + b, 0) / lats.length;
    const centerLon = lons.reduce((a, b) => a + b, 0) / lons.length;
    
    // Analyze terrain at search area center
    let terrainResult;
    try {
      terrainResult = await MAT.terrain.analyzeTerrain(
        centerLat, 
        centerLon, 
        activeObs,
        { radiusNm: 10 }  // Analyze 10 NM radius
      );
    } catch (err) {
      console.warn('Terrain analysis failed:', err);
      terrainResult = { mode: 'auto', source: 'error' };
    }
    
    // Determine effective terrain mode
    const effectiveTerrainMode = terrainResult.modeOverride || terrainResult.mode;
    
    // Update settings with terrain analysis result
    const enhancedSettings = {
      ...settings,
      terrainMode: effectiveTerrainMode !== 'auto' ? effectiveTerrainMode : (settings.terrainMode || 'auto')
    };
    
    // Run standard computation with terrain-adjusted settings
    const result = computeProbableArea({
      ...params,
      settings: enhancedSettings
    });
    
    // Add terrain analysis to result
    if (!result.error) {
      result.terrainAnalysis = {
        mode: effectiveTerrainMode,
        confidence: terrainResult.confidence,
        source: terrainResult.source,
        stats: terrainResult.stats,
        keywords: terrainResult.keywordWarnings || terrainResult.keywords || []
      };
      
      // Override terrainMode in result to show what was actually used
      result.terrainMode = effectiveTerrainMode;
    }
  ...
  /**
   * Pre-fetch terrain data for a search area
   * Call this when the user starts entering observations to warm the cache
   * 
   * @param {number} lat - Center latitude
   * @param {number} lon - Center longitude
   * @param {number} [radiusNm=15] - Radius to pre-fetch
   * @returns {Promise<Object>} Cache status
   */
  async function prefetchTerrainData(lat, lon, radiusNm = 15) {
    if (!window.MAT?.terrain?.queryElevations) {
      return { success: false, reason: 'Terrain module not available' };
    }
    
    try {
      // Generate a larger grid for pre-fetching
      const points = MAT.terrain.generateSampleGrid(lat, lon, radiusNm, 5);
      const elevations = await MAT.terrain.queryElevations(points, 5);
      const validCount = elevations.filter(e => e !== null).length;
      
      return {
        success: validCount > 0,
        pointsQueried: points.length,
        pointsCached: validCount,
        coverage: Math.round(validCount / points.length * 100) + '%'
      };
    } catch (err) {
  ...
  MAT.elt.DF_BEARING_CONSTANTS = DF_BEARING_CONSTANTS;
  MAT.elt.getTerrainAdjustedConstants = getTerrainAdjustedConstants;
  ...
  // Async/terrain-enhanced versions
  MAT.elt.computeProbableAreaAsync = computeProbableAreaAsync;
  MAT.elt.prefetchTerrainData = prefetchTerrainData;
