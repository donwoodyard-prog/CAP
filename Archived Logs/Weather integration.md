...
2. The enhanced `mat-weather.js` automatically:
   - Checks if FIS-B weather data is available
   - Uses FIS-B METARs/TAFs when available (offline/in-flight)
   - Falls back to API when FIS-B is not available or stale
   - Shows "FIS-B" source indicator on datalinked weather

3. FIS-B data flow:
```
UAT (978 MHz) → Stratux → Raw GDL90 → mat-fisb-weather.js → mat-weather.js
                                      (decodes FIS-B)      (uses as primary)
```

**Note**: FIS-B weather requires UAT reception, which means:
- Must be within range of ADS-B ground station (typically FL240 and below)
- Reception improves with altitude and clear line of sight
- Weather data updates every ~5 minutes per station

### Integration Checklist for FIS-B Weather:

1. ✅ `mat-fisb-weather.js` - Decodes FIS-B products from GDL90 uplink messages
2. ✅ `mat-weather.js` - Enhanced to check FIS-B first, fallback to API
3. ⏳ Wire up: Connect Stratux raw GDL90 stream to FIS-B decoder
4. ⏳ UI indicator: Show FIS-B availability in Weather tab
