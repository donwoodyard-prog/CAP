...
(function() {
  ...
  const DEMO_CONFIGS = [
    ...
    {
      id: 'form104-import-1',
      title: 'Form 104 Import',
      subtitle: 'WMIRS Mission Data Parser',
      description: 'Learn how to import mission data from CAP Form 104. This demo loads sample Form 104 text and walks you through the parsing process to auto-populate mission fields.',
      module: 'form104',
      icon: '📄',
      color: '#3182ce',
      scenarios: ['missing-cessna', 'missing-snowmobile'],
      defaultScenario: 'missing-cessna'
    }
    // ========================================================================
    // ADD NEW DEMOS HERE
    // ========================================================================
  ...
  const DEMO_DATA = {
    ...
    'form104-import-1': {
      type: 'file-based',
      description: 'Form 104 import demonstration with sample mission data',
      scenarios: {
        'missing-cessna': {
          name: 'Missing Aircraft Search - Cessna 172',
          description: 'Complete Form 104 for a SAR mission searching for a missing Cessna 172 near Asheville, NC.',
          difficulty: 'beginner',
          dataFile: 'data/demo/form104-missing-cessna.txt'
        },
        'missing-snowmobile': {
          name: 'Missing Snowmobiler - Mountain Terrain',
          description: 'SAR mission searching for a missing snowmobiler in Idaho mountain terrain with avalanche hazards.',
          difficulty: 'intermediate',
          dataFile: 'data/demo/form104-missing-snowmobile.txt'
        }
      }
    }
  };
  ...
  async function loadDemo(demoId, actions) {
    console.log('DEBUG: loadDemo called with demoId =', demoId);
    const demo = DEMO_CONFIGS.find(d => d.id === demoId);
    if (!demo) throw new Error('Demo not found: ' + demoId);
    console.log('DEBUG: Found demo config:', demo.id, 'module:', demo.module);
    ...
    const {
      ...
      importKmlKmz, importAdsbTrack,
      // Form 104 demo support
      setForm104Text, setShowForm104Modal, setForm104DemoMode
    } = actions;
    ...
    if (demo.module === 'commandTools') {
      ...
      if (dataFiles && dataFiles.length > 0) {
        ...
        // Set pendingAnalysis flag to trigger auto-analysis when Command Tools tab renders
        setCmdState(prev => ({
          ...prev,
    ...
    console.log('DEBUG: Checking eltAssist condition. demo.module =', demo.module);
    if (demo.module === 'eltAssist') {
      console.log('DEBUG: Entered eltAssist block');
      ...
      if (!importKmlKmz || !importAdsbTrack) {
        console.error('ELT Demo: importKmlKmz or importAdsbTrack not available');
        setDemoLoadStatus('Error: Import functions not available');
        return;
      }
      ...
      for (let i = 0; i < demo.dataFiles.length; i++) {
        ...
        const baseFilename = filename.split('/').pop();
        console.log('DEBUG: Loading file', i, ':', filename);
        setDemoLoadStatus('Loading: ' + baseFilename);
        ...
        try {
          ...
          // Create a File object from the blob (import functions expect File objects)
          const file = new File([blob], baseFilename, { 
            type: filename.toLowerCase().endsWith('.kmz') ? 'application/vnd.google-earth.kmz' : 'application/vnd.google-earth.kml+xml' 
          });
          
          console.log('DEBUG: Created File object:', file.name, 'size:', file.size);
          
          // Determine which import function to use based on filename
          if (baseFilename.toLowerCase().includes('flightaware') || baseFilename.toLowerCase().includes('adsb')) {
            console.log('DEBUG: Using importAdsbTrack for', baseFilename);
            await importAdsbTrack(file);
          } else {
            console.log('DEBUG: Using importKmlKmz for', baseFilename);
            await importKmlKmz(file);
          }
          
          loadedCount++;
          console.log('DEBUG: Successfully imported', baseFilename);
    ...
    // FORM 104 IMPORT DEMO
    if (demo.module === 'form104') {
      console.log('DEBUG: Entered form104 block');
      console.log('DEBUG: setForm104Text =', typeof setForm104Text);
      console.log('DEBUG: setShowForm104Modal =', typeof setShowForm104Modal);
      console.log('DEBUG: setForm104DemoMode =', typeof setForm104DemoMode);
      setDemoLoadStatus('Loading Form 104 sample data...');
      const demoData = DEMO_DATA[demo.id];
      
      if (!demoData || !demoData.scenarios) {
        setDemoLoadStatus('Error: Form 104 demo data not found');
        return;
      }
      ...
      if (!scenario || !scenario.dataFile) {
        setDemoLoadStatus('Error: Scenario data file not configured');
        return;
      }
      
      setDemoLoadStatus('Loading: ' + scenario.name);
      
      try {
        // Fetch the Form 104 text file
        const response = await fetch(scenario.dataFile);
        if (!response.ok) {
          throw new Error('Failed to load file: ' + response.status);
        }
        const form104Text = await response.text();
        
        // Set the form text and open the modal
        if (setForm104Text) {
          setForm104Text(form104Text);
        }
        if (setForm104DemoMode) {
          setForm104DemoMode(true);
        }
        if (setShowForm104Modal) {
          setShowForm104Modal(true);
        }
        
        setDemoLoadStatus('Form 104 data loaded!');
        await new Promise(r => setTimeout(r, 300));
        
      } catch (err) {
        console.error('Error loading Form 104 demo:', err);
        setDemoLoadStatus('Error: ' + (err.message || 'Failed to load Form 104 data'));
        return;
      }
    }
    ...
    const onLoadDemo = async (demoId) => {
      ...
      try {
        await loadDemo(demoId, {
          ...
          importKmlKmz: actionsObj.importKmlKmz,
          importAdsbTrack: actionsObj.importAdsbTrack,
          // Form 104 demo support (passed in actionsObj from index.html)
          setForm104Text: actionsObj.setForm104Text,
          setShowForm104Modal: actionsObj.setShowForm104Modal,
          setForm104DemoMode: actionsObj.setForm104DemoMode
        });
      } catch (err) {
